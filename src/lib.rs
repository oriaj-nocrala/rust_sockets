...
impl P2PMessenger {
    pub fn new(peer_name: String) -> P2PResult<Self> {
        let tcp_port = 6969;
        let discovery = DiscoveryService::new(peer_name.clone(), tcp_port)?;
    ...
    pub fn discover_peers(&self) -> P2PResult<Vec<crate::protocol::PeerInfo>> {
        self.discovery.request_peers()?;
        Ok(self.discovery.get_peers())
    }

    pub fn get_discovered_peers(&self) -> Vec<crate::protocol::PeerInfo> {
        self.discovery.get_peers()
    }

    pub async fn get_connected_peers(&self) -> Vec<crate::protocol::PeerInfo> {
        self.peer_manager.get_connected_peers().await
    }

    pub async fn connect_to_peer(&self, peer_info: &crate::protocol::PeerInfo) -> P2PResult<()> {
    ...
    pub fn save_received_file(&self, message: &Message) -> P2PResult<String> {
        if let crate::protocol::message::MessageContent::File { filename, data } = &message.content {
            ...
            fs::write(&file_path, data).map_err(P2PError::Network)?;
...
pub use crate::protocol::{message::MessageContent, PeerInfo};